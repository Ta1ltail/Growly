...
export function WeekdayPicker({
  ...
  const isWeekdays = value.length === 5 && WEEKDAYS.every((d) => value.includes(d));
  const isWeekends = value.length === 2 && WEEKENDS.every((d) => value.includes(d));
  ...
  return (
    <div>
          ...
          return (
            <button
              ...
              className={`flex size-9 items-center justify-center rounded-xl text-xs font-semibold transition-all active:scale-90 ${
                active ? "bg-accent text-white shadow-sm" : "bg-surface2 text-muted hover:text-ink"
              }`}
            >
              {WEEKDAY_LETTER[day]}
            </button>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        <Shortcut label="Every day" active={isEveryDay} onClick={() => onChange([0, 1, 2, 3, 4, 5, 6])} />
        <Shortcut label="Weekdays" active={isWeekdays} onClick={() => onChange([1, 2, 3, 4, 5])} />
        <Shortcut label="Weekends" active={isWeekends} onClick={() => onChange([0, 6])} />
      </div>
    </div>
  );
}
