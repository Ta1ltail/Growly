...
export function SystemInfoSection({ query }: { query: string }) {
  ...
  const rows: { label: string; value: React.ReactNode; terms: string }[] = [
    ...
    { label: "Online", value: String(dyn.online), terms: "network connection" },
    {
      ...
      terms: "hardware concurrency",
    },
    {
      label: "Device memory",
      value: navAny?.deviceMemory ? `${navAny.deviceMemory} GB` : "—",
      terms: "ram",
    },
    ...
    {
      label: "Pixel ratio",
      value: typeof window !== "undefined" ? window.devicePixelRatio : "—",
      terms: "dpr retina",
    },
    { label: "OS color scheme", value: dyn.colorScheme, terms: "dark light" },
    {
      label: "Prefers reduced motion",
      value: String(dyn.reducedMotion),
      terms: "accessibility",
    },
    ...
    {
      label: "App data size",
      value: bytesToSize(appBytes),
      terms: "storage localstorage",
    },
    { label: "Storage estimate", value: storage, terms: "quota usage disk" },
  ];

  const visible = rows.filter((r) =>
    matchQuery(query, `${r.label} ${r.terms}`),
  );
